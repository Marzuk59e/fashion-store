...
import { C, S, CATEGORIES, font } from "../constants/theme.js";
    ...
    <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: 24, marginBottom: 24 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 20px" }}>
        <FormField label="ID (number)">
          <input style={inputStyle} placeholder="Auto if empty" value={f.id}
        ...
        <FormField label="Product Name">
          <input style={inputStyle} placeholder="Draped Silk Blouse" value={f.name}
            onChange={e => set("name", e.target.value)}
            onFocus={e => e.target.style.borderColor = C.gold}
            onBlur={e => e.target.style.borderColor = C.border2} />
        </FormField>
        <FormField label="Brand">
          <input style={inputStyle} placeholder="Maison Élite" value={f.brand}
            onChange={e => set("brand", e.target.value)}
            onFocus={e => e.target.style.borderColor = C.gold}
            onBlur={e => e.target.style.borderColor = C.border2} />
        </FormField>
        <FormField label="Price (USD)">
          <input style={inputStyle} type="number" placeholder="245" value={f.price}
            onChange={e => set("price", e.target.value)}
            onFocus={e => e.target.style.borderColor = C.gold}
            onBlur={e => e.target.style.borderColor = C.border2} />
        </FormField>
        <FormField label="Badge (optional)">
          <input style={inputStyle} placeholder="New · Best Seller · Limited" value={f.badge}
            onChange={e => set("badge", e.target.value)}
            onFocus={e => e.target.style.borderColor = C.gold}
            onBlur={e => e.target.style.borderColor = C.border2} />
        </FormField>
        <FormField label="Sizes (comma-separated)">
          <input style={inputStyle} placeholder="XS,S,M,L,XL" value={f.sizes}
        ...
        <FormField label="BG Color 1">
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            ...
            <input style={{ ...inputStyle, flex: 1 }} value={f.bg1}
        ...
        <FormField label="BG Color 2">
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <input type="color" value={f.bg2} onChange={e => set("bg2", e.target.value)}
              style={{ width: 40, height: 38, border: `1px solid ${C.border2}`, borderRadius: 6, cursor: "pointer", background: "none", padding: 2 }} />
            <input style={{ ...inputStyle, flex: 1 }} value={f.bg2}
      ...
      <FormField label="Product Photo (required)">
        <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
          <input style={{ ...inputStyle, flex: 1 }} placeholder="https://... or upload below"
        ...
        {f.image && (
          <img src={f.image} alt=""
            style={{ width: 80, height: 80, objectFit: "cover", borderRadius: 8, marginTop: 10, border: `1px solid ${C.border2}` }}
